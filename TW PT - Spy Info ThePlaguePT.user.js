...
(() => {
    ...
    function metricCard(label, value, delta, inverse, baselineText, endLabel) {
        ...
        return `
            <div class="${APP.id}-metric" title="${escapeHTML(title)}">
                ...
                <strong>${escapeHTML(value)}</strong>
                ${breakdown}
                <em class="${deltaClass(delta, inverse)}">${escapeHTML(deltaPrefix + deltaText)}</em>
            </div>
        `;
    }
    ...
    function baselineValue(snapshot, key) {
        return snapshot && Number.isFinite(snapshot[key]) ? formatNumber(snapshot[key]) : "N/D";
    }

    function baselineRankValue(snapshot) {
        return snapshot && Number.isFinite(snapshot.rank) ? `#${formatNumber(snapshot.rank)}` : "N/D";
    }

    function baselineOdValue(snapshot, key) {
        const entry = snapshot && snapshot.od && snapshot.od[key];
        return entry && Number.isFinite(entry.score) ? formatNumber(entry.score) : "N/D";
    }
    ...
    function injectStyle() {
        ...
        style.textContent = `
            ...
            .${APP.id}-controlsGrid {
                display: grid;
                grid-template-columns: .75fr 1.55fr 1fr 1fr .62fr;
                gap: 8px;
                align-items: end;
            }
            ...
            .${APP.id}-metricBreakdown {
                display: grid;
                gap: 2px;
                margin-top: 4px;
                padding-top: 3px;
                border-top: 1px solid rgba(200, 144, 66, .55);
            }

            .${APP.id}-metricBreakdown small {
                display: flex;
                justify-content: space-between;
                gap: 6px;
                color: #5d3514;
...
(() => {
    ...
    function attachTribeLauncherToScriptBar(element) {
        const bar = ensureTribeScriptBar();
        if (!bar || !element) return;

        element.classList.add("tp-theplaguept-script-bar-item");
        element.setAttribute("data-tp-title", APP.displayTitle);
        element.setAttribute("aria-label", APP.displayTitle);
        element.removeAttribute("title");
    ...
    function createLauncher() {
        ...
        button.addEventListener("click", openPanel);
        document.body.appendChild(button);
        attachTribeLauncherToScriptBar(button);
        state.launcher = button;
        setupLauncherPosition();
    }
    ...
    function injectStyle() {
        ...
        style.textContent = `
            ...
            #tp-theplaguept-script-bar > #${APP.id}-launcher {
            ...
            #tp-theplaguept-script-bar > #${APP.id}-launcher:hover,
            #tp-theplaguept-script-bar > #${APP.id}-launcher:focus-visible {
                width: 30px !important;
                min-width: 30px !important;
                max-width: 30px !important;
                padding: 0 !important;
            ...
            #tp-theplaguept-script-bar > #${APP.id}-launcher .${APP.id}-launcherLabel,
            #tp-theplaguept-script-bar > #${APP.id}-launcher:hover .${APP.id}-launcherLabel,
            #tp-theplaguept-script-bar > #${APP.id}-launcher:focus-visible .${APP.id}-launcherLabel {
            ...
            #tp-theplaguept-script-bar > #${APP.id}-launcher[data-tp-title]::after {
            ...
            #tp-theplaguept-script-bar > #${APP.id}-launcher[data-tp-title]:hover::after,
            #tp-theplaguept-script-bar > #${APP.id}-launcher[data-tp-title]:focus-visible::after {
